...
fn test_workspace_isolation_persistence_isolation() {
    ...
    let frame_id2 = merkle::types::FrameID::from([3u8; 32]);
    {
        ...
        head_index.update_head(&node_id, "test", &frame_id2).unwrap();
    }
    {
        let head_index = ctx2.api().head_index().read();
    ...
    assert_ne!(persistence_path1, persistence_path2, "Persistence files should be in different locations");
    assert!(persistence_path1.exists(), "Workspace 1 persistence file should exist");
    assert!(persistence_path2.exists(), "Workspace 2 persistence file should exist");
    ...
    // Verify workspace 1 still has its data
    {
        ...
        let head = head_index.get_head(&node_id, "test").unwrap();
        assert_eq!(head, Some(frame_id), "Workspace 1 should have its original data");
    }
