...
use merkle::config::{MerkleConfig, ProviderConfig, ProviderType, PromptCache, resolve_prompt_path, xdg};
...
fn test_xdg_providers_dir() {
    ...
    assert_eq!(providers_dir, test_config_home.join("merkle").join("providers"));
...
fn test_resolve_prompt_path_tilde() {
    ...
    let resolved = resolve_prompt_path(path, &base_dir).unwrap();
    assert_eq!(resolved, PathBuf::from(home).join("prompts").join("test.md"));
}
...
"#,
        ...
        let provider_names: Vec<String> = all_providers.iter()
            .map(|p| p.provider_name.as_ref().cloned().unwrap_or_else(|| "<no name>".to_string()))
            .collect();
        
        assert!(!all_providers.is_empty(), "At least one provider should be loaded. Found: {:?}", provider_names);
        
        let provider = registry.get("test-ollama")
            .unwrap_or_else(|| panic!("Provider 'test-ollama' not found. Loaded providers: {:?}", provider_names));
...
fn test_load_order_xdg_overrides_config() {
    ...
    with_xdg_env(&test_dir, || {
        ...
        config.providers.insert("test-provider".to_string(), provider_config.clone());
